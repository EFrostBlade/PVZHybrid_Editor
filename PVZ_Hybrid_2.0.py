...
def overPlant(f):
    addr = (
        int.from_bytes(
            PVZ_data.PVZ_memory.read_bytes(0x40E2C2, 1)
            + PVZ_data.PVZ_memory.read_bytes(0x40E2C1, 1)
            + PVZ_data.PVZ_memory.read_bytes(0x40E2C0, 1)
            + PVZ_data.PVZ_memory.read_bytes(0x40E2BF, 1)
        )
        + 0x40E2C3
    )
    if f:
        ...
        PVZ_data.PVZ_memory.write_bytes(
            0x00843DEE, b"\xe9\xe1\xcb\xbc\xff\x0f\x1f\x00", 8
        )
        PVZ_data.PVZ_memory.write_bytes(
            0x00843E23, b"\xe9\xac\xcb\xbc\xff\x0f\x1f\x00", 8
        )
        PVZ_data.PVZ_memory.write_bytes(
            0x00843E58, b"\xe9\x77\xcb\xbc\xff\x0f\x1f\x00", 8
        )
        ...
        PVZ_data.PVZ_memory.write_bytes(0x0084A3EB, b"\xe9\x6a\x3f\xbc\xff\x90", 6)
        ...
        PVZ_data.PVZ_memory.write_bytes(
            0x00843DEE, b"\x85\xc0\x0f\x84\xde\xcb\xbc\xff", 8
        )
        PVZ_data.PVZ_memory.write_bytes(
            0x00843E23, b"\x85\xc0\x0f\x84\xa9\xcb\xbc\xff", 8
        )
        PVZ_data.PVZ_memory.write_bytes(
            0x00843E58, b"\x85\xc0\x0f\x84\x74\xcb\xbc\xff", 8
        )
        ...
        PVZ_data.PVZ_memory.write_bytes(0x0084A3EB, b"\x0f\x85\x69\x3f\xbc\xff", 6)
...
def column(f):
    ...
    global column2addr
    if f:
        column1addr = PVZ_data.PVZ_memory.read_bytes(0x00410ADF, 5)
        column2addr = PVZ_data.PVZ_memory.read_bytes(0x00439035, 5)
        ...
        PVZ_data.PVZ_memory.write_bytes(0x00410ADF, column1addr, 5)
        PVZ_data.PVZ_memory.write_bytes(0x00439035, column2addr, 5)


def unlock():
    addr = (
        int.from_bytes(
            PVZ_data.PVZ_memory.read_bytes(0x00453B24, 1)
            + PVZ_data.PVZ_memory.read_bytes(0x00453B23, 1)
            + PVZ_data.PVZ_memory.read_bytes(0x00453B22, 1)
            + PVZ_data.PVZ_memory.read_bytes(0x00453B21, 1)
        )
        + 0x00453B25
    )
...
def noSlot_operstion(noSlot_event):
    while not noSlot_event.is_set():
        try:
            start = PVZ_data.PVZ_memory.read_bool(
                PVZ_data.PVZ_memory.read_int(
                    PVZ_data.PVZ_memory.read_int(
            ...
            if start is True:
                PVZ_data.PVZ_memory.write_bool(
                    PVZ_data.PVZ_memory.read_int(
                        PVZ_data.PVZ_memory.read_int(
                            PVZ_data.PVZ_memory.read_int(PVZ_data.baseAddress) + 0x774
                        )
...
def setAllBullet(f, type):
    ...
    if f:
        ...
        PVZ_data.PVZ_memory.write_bytes(newmem_setAllBullet, shellcode, 17)
        PVZ_data.PVZ_memory.write_bytes(
            0x0046E8DC,
...
def bossHPDraw(f):
    ...
    if f:
        ...
        newmem_bossHPDraw = pymem.memory.allocate_memory(
            PVZ_data.PVZ_memory.process_handle, 512
        )
        ...
        PVZ_data.PVZ_memory.write_bytes(0x0086E54D, b"\x8b\x92\x68\x07\x00\x00", 6)
        pymem.memory.free_memory(
            PVZ_data.PVZ_memory.process_handle, newmem_resetBossNum
        )
        PVZ_data.PVZ_memory.write_bytes(0x0086E566, b"\x74\xeb\x80\x7b\x18\x00", 6)
        pymem.memory.free_memory(PVZ_data.PVZ_memory.process_handle, newmem_bossHPDraw)
        pymem.memory.free_memory(PVZ_data.PVZ_memory.process_handle, newmem_bossNum)
